...
namespace Dots.RVO {
    ...
    public struct ORCALinesJob : IJobParallelFor {
        ...
        [ReadOnly] public NativeArray<AgentData> m_inputAgents;
        [ReadOnly] public NativeArray<AgentTreeNode> m_inputAgentTree;

        [ReadOnly] public NativeArray<ObstacleInfos> m_staticObstacleInfos;
        [ReadOnly] public NativeArray<ObstacleVertexData> m_staticRefObstacles;
        [ReadOnly] public NativeArray<ObstacleVertexData> m_staticObstacles;
        [ReadOnly] public NativeArray<ObstacleTreeNode> m_staticObstacleTree;

        [ReadOnly] public NativeArray<ObstacleInfos> m_dynObstacleInfos;
        [ReadOnly] public NativeArray<ObstacleVertexData> m_dynRefObstacles;
        [ReadOnly] public NativeArray<ObstacleVertexData> m_dynObstacles;
        [ReadOnly] public NativeArray<ObstacleTreeNode> m_dynObstacleTree;
        ...
        public void Execute(int index) {
            ...
            m_OutputAgents[index] = agent;
            if (m_timestep <= 0) {
                m_timestep = 0.02f;
            }
            if (agent.IsJams) {
                agent.CurrentJamsTime += m_timestep;
                if (agent.CurrentJamsTime >= agent.DefaultJamsTime) {
                    agent.CurrentJamsTime = 0;
                    agent.IsJams = false;
                } else {
            ...
            if (m_staticObstacleTree.Length > 0) {
                ...
                for (int i = 0; i < staticObstacleNeighbors.Length; ++i) {
                    ...
                    for (int j = 0; j < m_orcaLines.Length; ++j) {
                        if (Det(invTimeHorizonObst * relPos1 - m_orcaLines[j].point, m_orcaLines[j].dir) -
                            invTimeHorizonObst * oRadius
                            >= -EPSILON &&
                            Det(invTimeHorizonObst * relPos2 - m_orcaLines[j].point, m_orcaLines[j].dir) -
                            invTimeHorizonObst * oRadius >= -EPSILON) {
                            alreadyCovered = true;
                            break;
                        }
                    }
                    ...
                    if (s < 0.0f && distSq1 <= radiusSq) {
                        ...
                        if (vertex.convex) {
                            ...
                            line.dir = normalizesafe(float2(-relPos1.y, relPos1.x));
                        ...
                        if (nextVertex.convex && Det(relPos2, nextVertex.dir) >= 0.0f) {
                            line.point = float2(0f);
                            line.dir = normalizesafe(float2(-relPos2.y, relPos2.x));
                            m_orcaLines.Add(line);
                        }
                        continue;
                    ...
                    if (s < 0.0f && distSqLine <= radiusSq) {
                        ...
                        lLegDir =
                            float2(relPos1.x * leg1 - relPos1.y * oRadius, relPos1.x * oRadius + relPos1.y * leg1) /
                            distSq1;
                        rLegDir = float2(relPos1.x * leg1 + relPos1.y * oRadius,
                            -relPos1.x * oRadius + relPos1.y * leg1) / distSq1;
                        ...
                        lLegDir =
                            float2(relPos2.x * leg2 - relPos2.y * oRadius, relPos2.x * oRadius + relPos2.y * leg2) /
                            distSq2;
                        rLegDir = float2(relPos2.x * leg2 + relPos2.y * oRadius,
                            -relPos2.x * oRadius + relPos2.y * leg2) / distSq2;
                    } else {
                        // Usual situation.
                        if (vertex.convex) {
                            float leg1 = sqrt(distSq1 - radiusSq);
                            lLegDir = float2(relPos1.x * leg1 - relPos1.y * oRadius,
                                relPos1.x * oRadius + relPos1.y * leg1) / distSq1;
                        } else {
                    ...
                    if ((t < 0.0f && tLeft < 0.0f) ||
                        ...
                        float2 unitW = normalizesafe(a_velocity - leftCutOff);
                        ...
                        float2 unitW = normalizesafe(a_velocity - rightCutOff);
                        line.dir = float2(unitW.y, -unitW.x);
                    ...
                    float distSqLeft = tLeft < 0.0f
                        ? float.PositiveInfinity
                        : lengthsq(a_velocity - (leftCutOff + tLeft * lLegDir));
                    float distSqRight = tRight < 0.0f
                        ? float.PositiveInfinity
                        : lengthsq(a_velocity - (rightCutOff + tRight * rLegDir));
            ...
            //QueryOtherAgentTreeRecursive(ref a_position,
            //    ref agent,
            //    0,
            //    ref attackrange,
            //    ref isSetTarget
            //    );
            int allAgentNeighbors = 0;
            QueryAgentTreeRecursive(
                ref a_position,
                ref agent,
                ref rangeSq, 0,
                ref agentNeighbors,
                ref allAgentNeighbors
            );
                ...
                if (distSq > cRadSq) {
                    ...
                    if (dotProduct1 < 0.0f && lengthsq(dotProduct1) > cRadSq * wLengthSq) {
                        ...
                        if (Det(relPos, w) > 0.0f) {
                            // Project on left leg.
                            line.dir = float2(relPos.x * leg - relPos.y * cRad, relPos.x * cRad + relPos.y * leg) /
                                       distSq;
                        } else {
                            // Project on right leg.
                            line.dir = -float2(relPos.x * leg + relPos.y * cRad, -relPos.x * cRad + relPos.y * leg) /
            ...
            //if (allAgentNeighbors == agent.maxNeighbors)
            //{
            //    if (slowlyCount == agent.maxNeighbors)
            //    {
            //        agent.IsJams = true;
            //    }
            //}
        ...
        private void QueryAgentTreeRecursive(
            ref float2 center, ref AgentData agent, ref float rangeSq, int node, ref NativeList<DVP> agentNeighbors,
            ref int allAgentNeighbors
        ) {
            ...
            if (treeNode.end - treeNode.begin <= AgentTreeNode.MAX_LEAF_SIZE) {
                ...
                for (int i = treeNode.begin; i < treeNode.end; ++i) {
                    ...
                    // if (a.campType == agent.campType)
                    // {
                    //     if ((a.layerOccupation & ~agent.layerIgnore) == 0)
                    //     {
                    //         continue;
                    //     }
                    // }
                    if (!a.collisionEnabled || (top < a.baseline || bottom > a.baseline + a.height)) {
                        continue;
                    }
                ...
                if (distSqLeft < distSqRight) {
                    if (distSqLeft < rangeSq) {
                        ...
                        if (distSqRight < rangeSq) {
                            QueryAgentTreeRecursive(ref center, ref agent, ref rangeSq, treeNode.right,
                                ref agentNeighbors, ref allAgentNeighbors);
                        }
                    }
                } else {
                    if (distSqRight < rangeSq) {
                        QueryAgentTreeRecursive(ref center, ref agent, ref rangeSq, treeNode.right, ref agentNeighbors,
                            ref allAgentNeighbors);
                        if (distSqLeft < rangeSq) {
                            QueryAgentTreeRecursive(ref center, ref agent, ref rangeSq, treeNode.left,
                                ref agentNeighbors, ref allAgentNeighbors);
                        }
                    }
                }
        ...
        private void QueryObstacleTreeRecursive(
            ...
            if (treeNode.end - treeNode.begin <= ObstacleTreeNode.MAX_LEAF_SIZE) {
                ...
                if (distSqLeft < distSqRight) {
                    if (distSqLeft < rangeSq) {
                        QueryObstacleTreeRecursive(ref center, ref agent, ref rangeSq, treeNode.left,
                            ref obstacleNeighbors,
                            ref obstacles, ref refObstacles, ref obstaclesInfos, ref kdTree);
                        if (distSqRight < rangeSq) {
                            QueryObstacleTreeRecursive(ref center, ref agent, ref rangeSq, treeNode.right,
                                ref obstacleNeighbors,
                                ref obstacles, ref refObstacles, ref obstaclesInfos, ref kdTree);
                        }
                    }
        ...
        private bool LP1(
            NativeList<ORCALine> lines, int lineNo, float radius, float2 optVel, bool dirOpt, ref float2 result
        ) {
        ...
        private int LP2(NativeList<ORCALine> lines, float radius, float2 optVel, bool dirOpt, ref float2 result) {
            if (dirOpt) {
                ...
                result = normalizesafe(optVel) * radius;
            } else {
            ...
            for (int i = beginLine, iCount = lines.Length; i < iCount; ++i) {
                ...
                if (Det(dirA, ptA - result) > distance) {
                    ...
                    for (int j = numObstLines; j < i; ++j) {
                        ...
                        line.dir = normalizesafe(dirB - dirA);
                        projLines.Add(line);
                    }
                    float2 tempResult = result;
