...
namespace Dots.RVO {
    ...
    public struct Segment2D {
        ...
        public Segment2D(float2 a, float2 b) {
            A = a;
            B = b;
        }
    ...
    public struct RaycastsJob : IJobParallelFor {
        ...
        [ReadOnly] public AxisPair m_plane;

        [ReadOnly] public float m_maxAgentRadius;

        [ReadOnly] public NativeArray<RaycastData> m_inputRaycasts;
        ...
        public void Execute(int index) {
            ...
            Segment2D raySegment =
                    new Segment2D(raycast.position, raycast.position + raycast.direction * raycast.distance),
                segment;
            ...
            if ((filter & RaycastFilter.OBSTACLE_STATIC) != 0) {
                ...
                if (m_staticObstacleTree.Length > 0)
                    QueryObstacleTreeRecursive(ref raycast, ref a_sqRange, 0, ref staticObstacleNeighbors,
                        ref m_staticObstacles, ref m_staticRefObstacles, ref m_staticObstacleInfos,
                        ref m_staticObstacleTree);
                NativeParallelHashMap<UIntPair, bool> coveredStaticEdges =
                    new NativeParallelHashMap<UIntPair, bool>(m_staticObstacleTree.Length * 2, Allocator.Temp);
                        ...
                        if (!alreadyCovered) {
                            if (raySegment.IsIntersecting(segment, out hitLocation)) {
                                ...
                                if (result.hitObstacle == -1
                                    || distancesq(r_position, hitLocation) <
                                    distancesq(r_position, result.hitObstacleLocation2D)) {
            ...
            if ((filter & RaycastFilter.OBSTACLE_DYNAMIC) != 0) {
                ...
                NativeParallelHashMap<UIntPair, bool> coveredDynEdges =
                    new NativeParallelHashMap<UIntPair, bool>(m_dynObstacleTree.Length * 2, Allocator.Temp);
            ...
            if ((filter & RaycastFilter.AGENTS) != 0) {
                ...
                float2 center, i1, i2, rayEnd = r_position + normalizesafe(r_dir) * raycast.distance;
                float
                    a_radius,
                    a_sqRadius,
                    cx,
            ...
            if (m_plane == AxisPair.XY) {
                ...
                if (result.hitAgent != -1)
                    result.hitAgentLocation =
                        float3(result.hitAgentLocation2D.x, baseline, result.hitAgentLocation2D.y);
                if (result.hitObstacle != -1)
                    result.hitObstacleLocation = float3(result.hitObstacleLocation2D.x, baseline,
                        result.hitObstacleLocation2D.y);
            }
            m_results[index] = result;
        }
        ...
        private void QueryAgentTreeRecursive(
            ref RaycastData raycast, ref float rangeSq, int node, ref NativeList<int> agentNeighbors
        ) {
